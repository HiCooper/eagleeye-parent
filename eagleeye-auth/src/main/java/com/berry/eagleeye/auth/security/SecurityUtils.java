...
import com.berry.eagleeye.auth.security.dto.UserBriefInfoDTO;
...
public final class SecurityUtils {
     ...
     */
    public static UserBriefInfoDTO getCurrentUser() {
        SecurityContext securityContext = SecurityContextHolder.getContext();
        return Optional.ofNullable(securityContext.getAuthentication())
                .map(authentication -> {
                    if (authentication.getPrincipal() instanceof UserBriefInfoDTO) {
                        return (UserBriefInfoDTO) authentication.getPrincipal();
                    }
                    return null;
                }).orElse(null);
    }
