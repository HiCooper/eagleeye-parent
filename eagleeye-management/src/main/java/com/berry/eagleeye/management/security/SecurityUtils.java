...
public final class SecurityUtils {
    ...
    public static List<String> getCurrentUserRole() {
        SecurityContext securityContext = SecurityContextHolder.getContext();
        return Objects.requireNonNull(Optional.ofNullable(securityContext.getAuthentication())
                .map(authentication ->
                        authentication.getAuthorities().stream().map(GrantedAuthority::getAuthority)).orElse(null)).collect(Collectors.toList());
    }
}
