...
import com.berry.eagleeye.module.StatusUpdateMessageDto;
 ...
 */
@Component
public class AlgMsgRecvHandler {
    ...
    public AlgMsgRecvHandler(IRecordDetailDaoService recordDetailDaoService) {
    ...
    public void processPassMsg(String message, Channel channel, @Header(AmqpHeaders.DELIVERY_TAG) long deliveryTag) {
        ...
        // process message
        ApprovalResultDto dto = null;
        try {
            dto = JSON.parseObject(message, ApprovalResultDto.class);
        } catch (Exception e) {
        ...
        if (dto != null && dto.getRecordId() != null) {
            ...
            if (recordDetail != null) {
                ...
                if (EnumConstant.ExecuteState.userCancel.name().equals(recordDetail.getProcessStatus())) {
                    logger.warn("该记录：【{}】 用户已取消预审，忽略该Pass消息", recordDetail.getRequestRecordId());
                } else {
        ...
        if (statusUpdateMessageDto != null) {
            ...
            if (isNoneBlank(recordId, status)) {
                ...
                if (recordDetail != null) {
                    // 状态检查
                    if (EnumConstant.ExecuteState.userCancel.name().equals(recordDetail.getProcessStatus())) {
                        logger.warn("该记录：【{}】 用户已取消预审，忽略该状态变更消息", recordDetail.getRequestRecordId());
                    } else {
